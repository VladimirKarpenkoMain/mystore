...
class UserRegistrationForm(UserCreationForm):
    ...
    email = forms.EmailField(widget=forms.EmailInput(attrs={
        'class': "form-control py-4", 'aria - describedby': "emailHelp", 'placeholder': "Введите адрес эл. почты"
    }))
...
class UserProfileForm(UserChangeForm):
    ...
    username = forms.CharField(widget=forms.TextInput(
        attrs={'class': 'form-control py-4', 'readonly': True, 'aria-describedby': "usernameHelp"}))
    email = forms.EmailField(widget=forms.EmailInput(
        attrs={'class': 'form-control py-4', 'readonly': True, 'aria-describedby': "emailHelp"}))
